...
//   • NO existe columna 'paid' — status='Pagado' indica pago hecho
//   • Status válidos: Pendiente, Confirmada, Completada, Pagado,
//     Reagendada, En espera, Solicitud, Cancelada, No asistió, Rechazada
// ══════════════════════════════════════════════════════════════════════
...
export async function getDashboardStats(
  ...
  // ── Queries paralelas (sin filtro 'paid' que no existe) ──
  const [
    todayApts,
    monthApts,
    paidThisMonth,
    ...
    // Citas pagadas/completadas del mes (todas cuentan como ingreso cobrado)
    supabase.from('appointments').select('date, service_cost, service_name, status').eq('user_id', userId).in('status', PAID_STATUSES).gte('date', monthStart).lte('date', monthEnd),
    supabase.from('appointments').select('service_cost').eq('user_id', userId).in('status', PAID_STATUSES).gte('date', lastMonthStart).lte('date', lastMonthEnd),
    ...
    supabase.from('appointments').select('id', { count: 'exact', head: true }).eq('user_id', userId).in('status', PAID_STATUSES),
    inactiveClientsQuery(supabase, userId),
    upcomingAppointmentsQuery(supabase, userId, today),
  ]);

  const paid = paidThisMonth.data || [];
  const monthRevenue = paid.reduce((s: number, a: any) => s + (a.service_cost || 0), 0);
  const lastMonthRevenue = (paidLastMonth.data || []).reduce((s: number, a: any) => s + (a.service_cost || 0), 0);

  // Pendiente: citas confirmadas/pendientes que aún no se cobran
  const pendingRevenue = (monthApts.data || [])
    .filter((a: any) => ['Confirmada', 'Pendiente'].includes(a.status))
  ...
  const completedThisMonth = monthApt.filter((a: any) => PAID_STATUSES.includes(a.status)).length;
  const avgTicket = completedThisMonth > 0 ? Math.round(monthRevenue / completedThisMonth) : 0;
  const completedLastMonth = (paidLastMonth.data || []).length;
  ...
  return {
    ...
    confirmedToday: todayApts.data?.filter((a: any) => a.status === 'Confirmada').length || 0,
    dailyRevenue,
    revenueByService,
