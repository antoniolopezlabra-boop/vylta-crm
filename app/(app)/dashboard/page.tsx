...
// ══════════════════════════════════════════════════════════════════════
// Dashboard premium — Brand Kit VYLTA v1.0 · Mayo 2026
//
// Estética dark-first siguiendo brand guidelines:
//   • Hero card con halo verde sutil + saludo + KPI grande del día
//   • Stat cards con número masivo + pictograma + trend
//   • Sidebar derecha con cumpleaños / inactivos / alerts
//   • Colores: hex exactos del brand kit (no vylta-amber-*, etc)
//
// Paleta usada:
//   verde   #10B981 → primario, ingresos, todo confirmado
//   ámbar   #F59E0B → cobros pendientes, alertas (uso restringido)
//   luxury  #A78BFA → solicitudes link público, semana
//   rose    #F43F5E → cumpleaños
// ══════════════════════════════════════════════════════════════════════
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <StatCard
            ...
            icon={CalendarCheck}
            accent="green"
            sub={stats.todayCount > 0 ? `${stats.todayConfirmed} confirmadas` : 'Día libre'}
          />
          <StatCard
            label="Pendientes"
            ...
            icon={Clock}
            accent="amber"
            sub={stats.todayPending > 0 ? 'Por confirmar' : 'Todas listas'}
          />
          <StatCard
            label="Cobrado hoy"
            ...
            icon={DollarSign}
            accent="green"
            sub={`${stats.todayPaidCount} ${stats.todayPaidCount === 1 ? 'cita pagada' : 'citas pagadas'}`}
          />
          <StatCard
            label="Mañana"
            value={`${stats.tomorrowCount}`}
            icon={CalendarCheck}
            accent={stats.tomorrowUnconfirmed > 0 ? 'amber' : 'green'}
            sub={stats.tomorrowUnconfirmed > 0 ? `${stats.tomorrowUnconfirmed} sin confirmar` : 'Todo en orden'}
          />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <StatCard
            ...
            icon={CalendarCheck}
            accent="luxury"
            sub={stats.weekCount > 0 ? 'En total' : 'Sin actividad'}
          />
          <StatCard
            label="Confirmadas"
          ...
          <StatCard
            ...
            icon={Clock}
            accent="amber"
            sub={stats.weekPending > 0 ? 'Por gestionar' : 'Todas listas'}
          />
          <StatCard
            label="Cobrado"
        ...
        <div className="space-y-6 lg:col-span-2">
            ...
            {stats.upcomingToday.length === 0 ? (
              ...
              <ul className="divide-y divide-border">
                {stats.upcomingToday.map((apt) => (
                  <AppointmentRow key={apt.id} apt={apt} showDate={false} />
                ))}
              </ul>
            )}
          </Section>
          ...
          {stats.upcomingFuture.length > 0 && (
            <Section
              ...
              count={stats.upcomingFuture.length}
            >
              <ul className="divide-y divide-border">
                {stats.upcomingFuture.map((apt) => (
                  <AppointmentRow key={apt.id} apt={apt} showDate={true} />
                ))}
              </ul>
            </Section>
          )}
          ...
          <div className="relative overflow-hidden rounded-xl border border-border bg-vylta-surface p-5 shadow-card">
            ...
            <div className="relative flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-bold text-vylta-bone">
                  Resumen de {stats.monthLabel}
                </h3>
                <p className="text-xs text-vylta-muted mt-0.5">
                  Llevas{' '}
                  <span className="font-bold text-vylta-bone tabular-nums">
              ...
              <Link
                href="/reportes"
                className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-vylta-green/10 px-3 py-2 text-xs font-bold text-vylta-green transition hover:bg-vylta-green/20"
              >
                Ver análisis
                <ArrowRight className="h-3 w-3" />
              </Link>
        ...
        <aside className="space-y-4">
          {stats.tomorrowUnconfirmed > 0 && (
            <AlertCard
              variant="warning"
              icon={AlertTriangle}
              title={`${stats.tomorrowUnconfirmed} ${stats.tomorrowUnconfirmed === 1 ? 'cita mañana' : 'citas mañana'} sin confirmar`}
              description="Confírmalas para reducir no-shows."
          ...
          {stats.upcomingBirthdays.length > 0 && (
            <SidebarCard
              icon={Cake}
              iconColor="text-vylta-rose"
              iconBg="bg-vylta-rose/10"
              iconRing="ring-vylta-rose/20"
              title="Cumpleaños esta semana"
              badge={stats.upcomingBirthdays.length}
              badgeClass="bg-vylta-rose/15 text-vylta-rose"
            >
                  ...
                  <li key={c.id} className="flex items-center justify-between gap-2">
                    ...
                    {c.phone && (
                      <a
                        ...
                        className="inline-flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-vylta-whatsapp/15 text-vylta-whatsapp transition hover:bg-vylta-whatsapp/25"
          ...
          {stats.inactiveClients.length > 0 && (
            <SidebarCard
              icon={UserX}
              iconColor="text-vylta-amber"
              iconBg="bg-vylta-amber/10"
              iconRing="ring-vylta-amber/20"
              title="Clientes inactivos"
              action={
                <Link
                  href="/clientes/inactivos"
                  className="text-[10px] font-semibold text-vylta-green hover:text-vylta-green-light transition-colors"
                >
                  Ver todos
                </Link>
              }
            >
              <p className="mb-3 text-[11px] text-vylta-muted">
                Sin visita hace 60+ días. ¡Ideal para reenganchar!
              </p>
              <ul className="space-y-2.5">
              ...
              <div className="relative overflow-hidden rounded-xl border border-vylta-green/25 bg-vylta-surface p-6 text-center">
                ...
                <div className="relative">
                  ...
                  <p className="mt-1 text-xs text-vylta-muted">
                    No tienes alertas pendientes ahora mismo.
                  </p>
                </div>
              </div>
            )}
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-border bg-vylta-surface shadow-card-lg">
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        ...
        <div
          ...
          style={{
            ...
            WebkitMaskImage: 'radial-gradient(ellipse 80% 50% at 50% 0%, black 30%, transparent 80%)',
          }}
        />
      </div>
      ...
      <div className="relative flex flex-col gap-5 p-6 sm:flex-row sm:items-end sm:justify-between sm:p-7">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-vylta-subtle">
            {todayLabel}
          </p>
          <h1 className="mt-2 text-3xl font-bold tracking-tightest text-vylta-bone sm:text-4xl">
            ...
            {firstName && (
              <>
                ,{' '}
                <span className="text-gradient-vylta">{firstName}</span>
              </>
            )}
          </h1>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm">
            <HeroStat
              icon={Calendar}
              value={todayCount}
              label={todayCount === 1 ? 'cita hoy' : 'citas hoy'}
              color="text-vylta-green"
            />
            {todayPending > 0 && (
              <HeroStat
                icon={Clock}
...
function HeroStat({
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className={cn('font-bold tabular-nums', color, !isCurrency && 'text-lg')}>
        {value}
      </span>
...
function SectionHeader({
  ...
  return (
    <div className="mb-3 flex items-center gap-2">
      ...
      <h2 className="text-[10px] font-bold uppercase tracking-[0.2em] text-vylta-muted">
        {label}
      </h2>
    </div>
  );
...
// ── Stat card premium con número masivo + glow del accent ──
function StatCard({
  label,
  value,
  icon: Icon,
  ...
  icon: any;
  sub: string;
  accent: 'green' | 'amber' | 'luxury' | 'rose';
}) {
  const colorMap = {
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl border border-border bg-vylta-surface p-4 shadow-card transition-all hover:border-border/80 hover:-translate-y-0.5">
      <div
        className="pointer-events-none absolute -top-10 -right-10 h-32 w-32 rounded-full blur-2xl opacity-30 transition-opacity group-hover:opacity-50"
        style={{ background: colorMap.haloHex }}
      />

      <div className="relative">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-vylta-subtle">
            {label}
          </span>
        ...
        <div className={cn('mt-3 text-3xl font-bold tabular-nums tracking-tightest', colorMap.text)}>
          {value}
        </div>
    ...
    <div className="rounded-xl border border-border bg-vylta-surface p-4 shadow-card">
      <div className="mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-vylta-bone">{title}</h3>
          {badge !== undefined && (
            <span className={cn('rounded-full px-1.5 py-0.5 text-[10px] font-bold tabular-nums', badgeClass)}>
              {badge}
            </span>
          )}
        </div>
...
// ── Status badges con hex oficiales VYLTA ──
...
function AppointmentRow({ apt, showDate }: { apt: TodayAppointment; showDate: boolean }) {
  return (
    <li>
      <Link
        href={`/citas/${apt.id}`}
        className="flex items-center gap-3 px-5 py-3.5 transition-colors hover:bg-vylta-card/40"
      >
        <div className="flex flex-col items-center justify-center rounded-lg bg-vylta-card/60 px-3 py-2 tabular-nums border border-border">
          <span className="text-sm font-bold leading-none text-vylta-bone">
            {apt.start_time?.slice(0, 5)}
          </span>
          {showDate && (
            <span className="mt-1 text-[9px] font-semibold uppercase tracking-wider text-vylta-subtle">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <span className="truncate text-sm font-semibold text-vylta-bone">
              {apt.client_name}
            </span>
            <span
              className={cn(
                'shrink-0 rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider',
                STATUS_STYLES[apt.status] || 'bg-vylta-card text-vylta-muted',
              )}
            >
              {apt.status}
            </span>
          </div>
          <div className="mt-0.5 truncate text-xs text-vylta-muted">
            {apt.service_name}
            {apt.staff ? ' · ' + apt.staff.name : ''}
          </div>
        </div>
        {apt.service_cost ? (
          <span className="shrink-0 text-sm font-bold tabular-nums text-vylta-green">
...
function EmptyDay() {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="mt-1 max-w-xs text-xs text-vylta-muted">
        Disfruta tu día libre o crea una cita para empezar.
      </p>
    </div>
  );
}
...
function AlertCard({
  ...
  const styles = isWarning
    ? 'border-vylta-amber/30 bg-vylta-amber/5'
    : 'border-vylta-luxury/30 bg-vylta-luxury/5';
    ...
    <div className={cn('rounded-xl border p-4 shadow-card', styles)}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <Link
            href={actionHref}
            className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-vylta-green hover:text-vylta-green-light transition-colors"
          >
            {actionLabel}
            <ArrowRight className="h-3 w-3" />
          </Link>
