...
//
// Filosofía: lo que necesitas HOY para operar tu negocio. El análisis
// histórico profundo vive en /reportes (con un link directo desde aquí).
//
// Layout:
//   ┌─ Saludo + fecha de hoy
//   ├─ KPIs de HOY: citas, confirmadas, pendientes, ingresos cobrados
//   ├─ Acciones rápidas (Nueva cita, Nuevo cliente)
//   │
//   ├── (columna izquierda 2/3) ────────────────────────────┐
//   │   Agenda de HOY (citas del día)                            │
//   │   Próximas citas futuras                                   │
//   │   Mini resumen del mes → "Ver análisis completo"           │
//   │                                                            │
//   └── (columna derecha 1/3) ────────────────────────────┐
//       Alertas:                                                  │
//       ⚠️ Citas de mañana sin confirmar                          │
...
export default async function DashboardPage() {
  ...
  const hasUrgentAlerts = stats.tomorrowUnconfirmed > 0;
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <Section
            ...
            action={
              <Link href="/citas" className="text-xs font-semibold text-vylta-green-600 hover:underline dark:text-vylta-green-400">
                Ver calendario →
              </Link>
            }
          >
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground">
                  Llevas <span className="font-bold text-foreground">{formatCurrency(stats.monthRevenue)}</span> cobrados
                  {' '}en{' '}<span className="font-bold text-foreground">{stats.monthAppointments}</span> citas este mes.
                </p>
              </div>
              <Link
                href="/reportes"
                className="inline-flex shrink-0 items-center gap-1.5 rounded-lg bg-vylta-green-500/10 px-3 py-1.5 text-xs font-bold text-vylta-green-700 transition hover:bg-vylta-green-500/20 dark:text-vylta-green-400"
              >
                Ver análisis
        ...
        {/* Columna derecha 1/3 - Alertas */}
        <aside className="space-y-4">
          {/* Alerta: citas mañana sin confirmar */}
          ...
          {stats.upcomingBirthdays.length > 0 && (
            <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="rounded-full bg-pink-500/10 px-1.5 py-0.5 text-[10px] font-bold text-pink-600 dark:text-pink-400">
                  ...
                  <li key={c.id} className="flex items-center justify-between gap-2">
                    ...
                    {c.phone && (
                      <a
                        ...
                        className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-vylta-green-500/10 text-vylta-green-600 transition hover:bg-vylta-green-500/20 dark:text-vylta-green-400"
          ...
          {stats.inactiveClients.length > 0 && (
            <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
              <div className="mb-3 flex items-center justify-between">
                ...
                <Link href="/clientes?segment=inactivos" className="text-[10px] font-semibold text-vylta-green-600 hover:underline dark:text-vylta-green-400">
                  Ver todos
                </Link>
              </div>
          ...
          {/* Si no hay alertas */}
          {!hasUrgentAlerts && stats.upcomingBirthdays.length === 0 && stats.inactiveClients.length === 0 && (
...
function BigKpi({
  ...
  icon: any;
  sub: string;
  accent: 'green' | 'amber' | 'indigo' | 'rose';
}) {
  const colors = {
  ...
  return (
    <div className={cn('rounded-xl border bg-card p-4 shadow-sm', colors.ring)}>
      <div className="flex items-center justify-between">
        ...
        <div className={cn('flex h-7 w-7 items-center justify-center rounded-md', colors.bg, colors.text)}>
...
function AppointmentRow({ apt, showDate }: { apt: TodayAppointment; showDate: boolean }) {
  return (
    <li className="flex items-center gap-3 px-4 py-3 transition hover:bg-secondary/40">
...
function AlertCard({
  ...
  const styles = variant === 'warning'
    ? 'border-vylta-amber-500/40 bg-vylta-amber-500/5'
    : 'border-vylta-indigo-500/40 bg-vylta-indigo-500/5';
  const iconStyles = variant === 'warning'
    ? 'text-vylta-amber-700 dark:text-amber-400'
    ...
    <div className={cn('rounded-xl border p-4 shadow-sm', styles)}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <Link
            href={actionHref}
            className="mt-2 inline-flex items-center gap-1 text-xs font-bold text-vylta-green-600 hover:underline dark:text-vylta-green-400"
          >
            {actionLabel} →
          </Link>
        </div>
